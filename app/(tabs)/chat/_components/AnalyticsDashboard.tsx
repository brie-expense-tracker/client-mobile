...
export default function AnalyticsDashboard() {
	...
	const [eventsNeedingAttention, setEventsNeedingAttention] = useState(
		analyticsService.getEventsNeedingAttention()
	);
	...
	const exportAnalytics = () => {
		...
		logger.debug('📊 [Analytics] Export:', data);
			...
			<ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
				...
				<View style={styles.section}>
					...
					<View style={styles.metricsGrid}>
						...
						<View style={styles.metric}>
							<Text style={styles.metricValue}>
								{(summary.satisfactionRate * 100).toFixed(0)}%
							</Text>
							<Text style={styles.metricLabel}>Satisfaction</Text>
						</View>
						<View style={styles.metric}>
							<Text style={styles.metricValue}>
								{(summary.groundingUsage * 100).toFixed(0)}%
							</Text>
						...
						{eventsNeedingAttention.slice(0, 5).map((event, index) => (
							<View key={index} style={styles.attentionItem}>
								...
								<Text style={styles.attentionDetails}>
									...
									{event.groundingConfidence &&
										event.groundingConfidence < 0.5 &&
										'❓ Low confidence'}
								</Text>
							</View>
						))}
				...
				<View style={styles.section}>
					...
					<Text style={styles.intentText}>
						Most common:{' '}
						<Text style={styles.intentHighlight}>
							{summary.mostCommonIntent || 'None'}
						</Text>
					</Text>
					<Text style={styles.intentText}>
						Fallback rate:{' '}
						<Text style={styles.intentHighlight}>
							{(summary.fallbackRate * 100).toFixed(1)}%
						</Text>
					</Text>
				</View>
