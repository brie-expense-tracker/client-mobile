...
export default function StructuredResponse({
	...
	// Don't render if there are no cards to show
	...
	const renderCard = (card: any, index: number) => {
		switch (card.type) {
				...
				return (
					<View key={index} style={styles.tableCard}>
						<View style={styles.tableHeader}>
							...
							<Text style={styles.cardTitle}>Spending Plan</Text>
						</View>
						<View style={styles.tableContent}>
							{card.data.headers && (
								<View style={styles.tableRow}>
									{card.data.headers.map((header: string, i: number) => (
							...
							{card.data.rows?.map((row: any[], rowIndex: number) => (
									...
									{row.map((cell: any, cellIndex: number) => (
										<Text
											...
											style={[
												...
												cellIndex === row.length - 1 && styles.tableAmountText,
											]}
										>
											{cell}
										</Text>
				...
				return (
					<View key={index} style={styles.checklistCard}>
						...
						<View style={styles.checklistContent}>
							{card.data.items?.map((item: any, itemIndex: number) => (
			...
			case 'setup':
				return (
					<View key={index} style={styles.setupCard}>
						<View style={styles.setupHeader}>
							<View style={styles.setupTitleContainer}>
								<Ionicons name="settings" size={18} color="#3b82f6" />
								<Text style={styles.setupTitle}>{card.data.title}</Text>
							</View>
							<View style={styles.setupProgress}>
								<Text style={styles.setupProgressText}>
									{card.data.completedItems}/{card.data.totalItems}
								</Text>
							</View>
						</View>
						<Text style={styles.setupDescription}>{card.data.description}</Text>
							...
							{card.data.items?.map((item: any, itemIndex: number) => (
								...
								>
									<View style={styles.setupItemContent}>
										<Ionicons
											name={
												item.done ? 'checkmark-circle' : 'add-circle-outline'
											}
											size={20}
											color={item.done ? '#10b981' : '#3b82f6'}
										/>
										<Text
											style={[
												styles.setupItemText,
												item.done && styles.setupItemTextDone,
											]}
										>
											{item.label}
										</Text>
	...
	const getContextualMessage = () => {
		if (hasBudgetCards && budgetCard?.data?.pressured) {
			const { name, percentage } = budgetCard.data.pressured;
			if (percentage >= 90) {
...
const styles = StyleSheet.create({
	...
	scrollContent: {
		flexGrow: 1,
		// Remove extra padding - let parent handle spacing
	},
	message: {
		fontSize: 16,
		fontWeight: '500',
	...
	cardsContainer: {
		marginBottom: 8,
		// Natural height - ScrollView handles overflow
	},
	cardsContent: {
		paddingRight: 16,
	},
