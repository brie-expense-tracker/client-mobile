...
export function useBulletproofStream({
	...
	const startStream = useCallback(
		async (
			...
			console.log('[BulletproofStream] Starting stream:', {
				messageId,
				sessionId,
			...
			try {
				...
				console.log('[BulletproofStream] Connecting to:', url);
				...
				const connection = startStreaming({
					...
					onDelta: (text: string) => {
						console.log(
							'[BulletproofStream] Delta received:',
							text.length,
							'chars'
						);
					...
					onDone: () => {
						console.log('[BulletproofStream] Stream completed');
					...
					onError: (error: string) => {
						console.error('[BulletproofStream] Stream error:', error);
						...
						console.log(
							'[BulletproofStream] Error categorized as:',
							errorState
						);
						...
						if (shouldRetry) {
							...
							console.log(
								`[BulletproofStream] Retrying in ${delay}ms (attempt ${nextRetryCount}/${retryConfig.maxRetries})`
							);
					...
					onMeta: (data: any) => {
						console.log('[BulletproofStream] Meta received:', data);
				...
				console.error('[BulletproofStream] Failed to start stream:', error);
	...
	const stopStream = useCallback(() => {
		console.log('[BulletproofStream] Stopping stream');
