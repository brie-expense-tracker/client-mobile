...
import { FactPack, FactPackBuilder, FactPackCalculator } from '../../components/assistant/factPack';
...
export class GroundingLayerService {
  ...
  generateFactPack(
    ...
    const averageDaily = FactPackCalculator.calculateDailyAverage(totalSpent, daysInPeriod);
    ...
    const builder = new FactPackBuilder()
      ...
      .setGoals(goals.map(g => ({
        ...
        remaining: 0, // Will be calculated by builder
        deadline: g.deadline ? new Date(g.deadline).toISOString() : new Date(now.getTime() + 365 * 24 * 60 * 60 * 1000).toISOString(),
  ...
  async generateGroundedResponse(
    ...
    // Check cache first
    const cacheKey = this.generateCacheKey(userQuestion, factPack.metadata.hash, intent);
    ...
    const response = await this.composeGroundedResponse(userQuestion, factPack, intent);
  ...
  private async composeGroundedResponse(
    ...
    if (question.includes('budget') || question.includes('spending') || question.includes('limit')) {
      ...
      calculations.push({
        ...
        output: { totalRemaining, utilization: Math.round((totalSpent / totalBudget) * 100) },
        method: 'sum_aggregation'
      });
      
      if (question.includes('how much left') || question.includes('remaining')) {
        response = `For ${factPack.time_window.period}, you have $${totalRemaining.toFixed(2)} remaining from your $${totalBudget.toFixed(2)} total budget.`;
      } else if (question.includes('over') || question.includes('exceed')) {
        const overBudgets = budgetData.filter(b => b.status === 'over');
        if (overBudgets.length > 0) {
          response = `You're over budget in ${overBudgets.length} category${overBudgets.length > 1 ? 's' : ''}: ${overBudgets.map(b => b.name).join(', ')}.`;
        } else {
          response = `You're currently under budget in all categories for ${factPack.time_window.period}.`;
        }
      } else {
        response = `Your budget status for ${factPack.time_window.period}: $${totalSpent.toFixed(2)} spent, $${totalRemaining.toFixed(2)} remaining.`;
      }
    }
    
    // Goal-related questions
    else if (question.includes('goal') || question.includes('progress') || question.includes('save')) {
      const goalData = factPack.goals;
      if (goalData.length === 0) {
        response = "You don't have any financial goals set up yet. Would you like me to help you create one?";
        confidence = 0.8;
      } else {
        const avgProgress = goalData.reduce((sum, g) => sum + g.progress, 0) / goalData.length;
        const behindGoals = goalData.filter(g => g.status === 'behind');
        
        calculations.push({
          type: 'goal_progress',
          input: { goals: goalData.length },
          output: { averageProgress: Math.round(avgProgress), behindCount: behindGoals.length },
          method: 'average_calculation'
        });
        
        if (question.includes('progress')) {
          response = `Your goals are ${Math.round(avgProgress)}% complete on average. ${behindGoals.length > 0 ? `${behindGoals.length} goal${behindGoals.length > 1 ? 's are' : ' is'} behind schedule.` : 'All goals are on track!'}`;
        } else {
          response = `You have ${goalData.length} financial goal${goalData.length > 1 ? 's' : ''} with an average progress of ${Math.round(avgProgress)}%.`;
        }
      }
    }
    
    // Spending pattern questions
    else if (question.includes('spend') || question.includes('pattern') || question.includes('trend')) {
      const patterns = factPack.spendingPatterns;
      
      calculations.push({
        type: 'spending_analysis',
        input: { totalSpent: patterns.totalSpent, days: 30 },
        output: { averageDaily: patterns.averageDaily, topCategories: patterns.topCategories.length },
        method: 'daily_average_calculation'
      });
      
      if (question.includes('daily') || question.includes('average')) {
        response = `Your average daily spending is $${patterns.averageDaily.toFixed(2)} for ${factPack.time_window.period}.`;
      } else if (question.includes('category') || question.includes('top')) {
        const topCategory = patterns.topCategories[0];
        if (topCategory) {
          response = `Your top spending category is ${topCategory.name} at $${topCategory.total.toFixed(2)} (${topCategory.percentage}% of total spending).`;
        } else {
          response = "I don't have enough transaction data to identify spending categories yet.";
        }
      } else {
        response = `Your spending pattern for ${factPack.time_window.period}: $${patterns.totalSpent.toFixed(2)} total, averaging $${patterns.averageDaily.toFixed(2)} per day.`;
      }
    }
    
    // Balance questions
    else if (question.includes('balance') || question.includes('account') || question.includes('money')) {
      ...
      response = `Your total balance across ${balanceData.length} account${balanceData.length > 1 ? 's' : ''} is $${totalBalance.toFixed(2)}.`;
    }
    
    // General financial health
    else if (question.includes('how am i doing') || question.includes('financial health') || question.includes('overview')) {
      const budgetUtilization = factPack.budgets.reduce((sum, b) => sum + b.utilization, 0) / factPack.budgets.length;
      const goalProgress = factPack.goals.reduce((sum, g) => sum + g.progress, 0) / factPack.goals.length;
      
      calculations.push({
        type: 'financial_health',
        input: { budgets: factPack.budgets.length, goals: factPack.goals.length },
        output: { avgBudgetUtilization: Math.round(budgetUtilization), avgGoalProgress: Math.round(goalProgress) },
        method: 'average_calculation'
      });
      
      let healthStatus = 'good';
      if (budgetUtilization > 80 || goalProgress < 30) healthStatus = 'needs_attention';
      else if (budgetUtilization > 60 || goalProgress < 50) healthStatus = 'moderate';
      
      response = `Your financial health for ${factPack.time_window.period} is ${healthStatus}. Budget utilization: ${Math.round(budgetUtilization)}%, Goal progress: ${Math.round(goalProgress)}%.`;
    }
  ...
  private generateCacheKey(userQuestion: string, factPackHash: string, intent: string): string {
    const canonicalizedQuery = userQuestion.toLowerCase()
      .replace(/[^\w\s]/g, '')
      .replace(/\s+/g, ' ')
      .trim();
