...
export class EnhancedCriticService {
	...
	private async runRuleValidators(
		...
		// Numeric guardrails
		const numericGuardrails = await this.validateNumericGuardrails(
			message,
			context
		);
	...
	private async runMiniCritic(
		...
		const isValid =
			!claimTypes.hasForbiddenPhrasing && !hasAmbiguity && !hasHallucination;
	...
	private async validateNumericGuardrails(
		...
		const sumsMatchFactPack = await this.validateSumsMatchFactPack(
			message,
			context
		);
		const datesInsideWindow = this.validateDatesInsideWindow(message);
		const budgetLimitsRespected = this.validateBudgetLimitsRespected(
			message,
			context
		);
	 ...
	 */
	private async validateSumsMatchFactPack(
		message: string,
		context: any
	): Promise<boolean> {
		...
		for (const match of totalMatches) {
			const mentionedTotal = parseFloat(
				match.match(/\$(\d+(?:\.\d{2})?)/)?.[1] || '0'
			);
	 ...
	 */
	private validateBudgetLimitsRespected(
		message: string,
		context: any
	): boolean {
		...
		for (const match of spendingMatches) {
			const suggestedSpending = parseFloat(
				match.match(/\$(\d+(?:\.\d{2})?)/)?.[1] || '0'
			);
	...
	private detectAmbiguity(message: string, context: any): boolean {
		...
		const matches = ambiguityIndicators.filter((pattern) =>
	...
	private detectHallucination(message: string, context: any): boolean {
		...
		const matches = hallucinationIndicators.filter((pattern) =>
			pattern.test(message)
		);
	...
	private isHighStakesTask(query: string): boolean {
		...
		const keywordMatches = highStakesKeywords.filter((keyword) =>
