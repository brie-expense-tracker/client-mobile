...
describe('routeModel - Hybrid Cost Optimization', () => {
	...
	describe('groundWithTools', () => {
		it('should extract facts for GET_BALANCE intent', async () => {
			const result = await groundWithTools(
				'What is my balance?',
				'GET_BALANCE',
				mockContext
			);
		...
		it('should extract facts for GET_BUDGET_STATUS intent', async () => {
			...
			expect(result.facts).toContain(
				'Entertainment: $50.00 remaining of $200.00'
			);
		});
	});
	...
	describe('miniModelWrite', () => {
		it('should generate message within token limit', async () => {
			const facts = [
				'You have 2 active budgets',
				'Total budget amount: $700.00',
			];
			const result = await miniModelWrite(
				'What is my balance?',
				'GET_BALANCE',
				facts
			);
	...
	describe('miniCriticValidate', () => {
		it('should validate safe messages', async () => {
			...
			expect(result.claimTypes.forbiddenClaims).toHaveLength(0);
			expect(result.confidence).toBeGreaterThan(0.8);
		});

		it('should detect forbidden investment advice', async () => {
			const message =
				'You should invest your money in stocks for better returns.';
			const facts = ['You have savings'];

			const result = await miniCriticValidate(message, facts);

			expect(result.isValid).toBe(false);
			expect(result.claimTypes.forbiddenClaims).toContain(
				'Contains forbidden pattern: \\bshould.*invest.*money'
			);
		});
	...
	describe('proModelStrategy', () => {
		it('should generate strategy for planning requests', async () => {
			const facts = [
				'Recent 30-day spending: $500.00',
				'You have 2 active budgets',
			];
			const userAsk = 'Give me a strategy to optimize my spending';

			const result = await proModelStrategy(
				'How to optimize?',
				'FORECAST_SPEND',
				facts,
				userAsk
