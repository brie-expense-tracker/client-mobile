...
export class EnhancedIntentMapper {
	...
	private confidenceHistory: { timestamp: number; confidence: number }[] = [];
	...
	async detectMultiLabelIntents(
		...
		const intentPatterns = [
			{
				intent: 'OVERVIEW' as Intent,
				patterns: [
					/tell me about (my )?finances?/i,
					/how am i doing/i,
					/summary|overview|health check|report/i,
					/(what|how) (does|do) my (budget|spending|goals) look like/i,
					/give me.*overview|show me.*summary/i,
					/how.*financial.*health|financial.*status/i,
					/what.*financial.*picture|money.*situation/i,
				],
				baseScore: 0.85,
			},
			{
			...
			{
				intent: 'GET_SPENDING_PLAN' as Intent,
				patterns: [
					/(?:how|where)\s+(?:should|do|to)\s+(?:i\s+)?(?:spend|allocate|use)\s+(?:my\s+)?(?:money|pay|paycheck|income)/i,
					/spending\s+plan|budget\s+allocation|spending\s+strategy/i,
					/50\s*\/?\s*30\s*\/?\s*20|needs.*wants.*savings|spending\s+breakdown/i,
				],
				baseScore: 0.8,
			},
			{
				intent: 'GOAL_ALLOCATION' as Intent,
				patterns: [
					/how\s+much\s+(?:should|to)\s+(?:i\s+)?(?:put|allocate|contribute)\s+(?:to|into)\s+(?:each|my)\s+goal/i,
					/goal\s+(?:allocation|distribution|priority|order)/i,
				],
				baseScore: 0.8,
			},
			{
				intent: 'SURPLUS_ALLOCATION' as Intent,
				patterns: [
					/(surplus|leftover|extra)\s+(money|cash|income)/i,
					/after\s+all\s+.*(goal|goals).*(reached|done|met)/i,
					/where\s+should\s+(?:i\s+)?put\s+(?:my\s+)?money\s+after/i,
				],
				baseScore: 0.75,
			},
			{
				...
				patterns: [
					...
					/what'?s.*rule|what.*is|define|definition/,
				],
				baseScore: 0.6,
			},
			{
				intent: 'HOWTO' as Intent,
				patterns: [
					/how.*do.*i|how.*to|how.*can.*i|how.*should.*i/,
					/step.*by.*step|walk.*through|tutorial/,
				],
				baseScore: 0.7,
			},
			{
				intent: 'DEFINITIONS' as Intent,
				patterns: [
					/what.*is|what.*does.*mean|define|definition|explain.*term/,
					/apr|apy|compound.*interest|emergency.*fund|budget.*rule/,
				],
				baseScore: 0.8,
			},
			{
				intent: 'APP_NAV' as Intent,
				patterns: [
					/mark.*paid|create.*budget|add.*goal|edit.*budget|link.*bank/,
					/categorize|pause.*subscription|spending.*breakdown/,
				],
				baseScore: 0.8,
			},
			{
				intent: 'MATH_QUICK' as Intent,
				patterns: [
					/if.*save.*month|how.*much.*save|calculate|percentage.*of/,
					/monthly.*to.*yearly|emergency.*fund.*amount|debt.*payoff/,
				],
				baseScore: 0.7,
			},
			{
				intent: 'HYSA_RECOMMENDATIONS' as Intent,
				patterns: [
					/\b(best|which|recommend|suggest|top|options?)\b.*\b(high[-\s]?yield\s+savings|hysa)\b/i,
					/\bhysa\b.*\b(pick|choose|options?)\b/i,
					// NEW: "banks/accounts" phrasing, still HYSA context
					/\b(banks?|credit unions?|accounts?)\b.*\b(should\s+i\s+look|look(?:ing)?\s+at|consider|recommend|suggest)\b/i,
				],
				baseScore: 0.85,
			},
		];
	...
	private applyContextBoost(
		...
		switch (intent) {
			case 'GET_BALANCE':
				if ((context.budgets?.length ?? 0) > 0) boost += 0.1;
				if ((context.transactions?.length ?? 0) > 0) boost += 0.05;
				break;
			case 'GET_BUDGET_STATUS':
				if ((context.budgets?.length ?? 0) > 0) boost += 0.15;
				break;
			case 'FORECAST_SPEND':
				if ((context.transactions?.length ?? 0) > 10) boost += 0.1;
				if ((context.budgets?.length ?? 0) > 0) boost += 0.05;
				break;
			case 'GET_GOAL_PROGRESS':
				if ((context.goals?.length ?? 0) > 0) boost += 0.15;
				break;
			case 'LIST_SUBSCRIPTIONS':
				if ((context.transactions?.length ?? 0) > 5) boost += 0.1;
				break;
			case 'GET_SPENDING_PLAN':
				if (context.profile?.monthlyIncome) boost += 0.1;
				if ((context.recurringExpenses?.length ?? 0) > 0) boost += 0.1;
				break;
			case 'GOAL_ALLOCATION':
				if ((context.goals?.length ?? 0) > 0) boost += 0.15;
				if (context.profile?.monthlyIncome) boost += 0.05;
				break;
			case 'HYSA_RECOMMENDATIONS':
				if (
					context?.sessionContext?.currentFocus === 'HYSA' &&
					(!context.sessionContext.focusExpiry ||
						Date.now() < context.sessionContext.focusExpiry)
				) {
					boost += 0.2; // NEW
				}
				break;
	...
	getUnknownIntentClarification(): {
		question: string;
		choices: { label: string; intent: Intent }[];
	} {
		return {
