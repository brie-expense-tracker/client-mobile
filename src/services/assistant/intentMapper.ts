		...
		extraContext: (context: FinancialContext, primaryData: any) => {
			if (primaryData.pressured) {
				...
				const otherCategories = primaryData.statuses.filter(
					(s: any) => s.name !== primaryData.pressured.name && s.percentage < 50
				);

				if (otherCategories.length > 0) {
					const potentialSavings = otherCategories.reduce(
						(sum: any, s: any) => {
	...
	LIST_SUBSCRIPTIONS: {
		primaryData: (context: FinancialContext) => {
			...
			const subscriptions = recurring.map((exp) => ({
				...
				utility: (exp as any).utility || 'medium', // low, medium, high
...
const YES_PATTERNS =
	/\b(yes|yeah|yep|sure|ok(ay)?|please|do it|go ahead|sounds good|absolutely|definitely|of course)\b/i;
const NO_PATTERNS =
	/\b(no|nope|nah|not now|cancel|stop|don't|do not|never mind|skip)\b/i;

/**
 * Route user input to appropriate intent, including confirmation detection
 */
export function routeIntent(
	userText: string,
	conversationId: string
): { mode: 'CHAT' | 'ACTIONS'; intent: IntentType } {
	const pending = ConversationState.getPendingAction(conversationId);
	...
	if (pending) {
		...
		if (NO_PATTERNS.test(userText)) {
			...
			return { mode: 'CHAT', intent: 'DECLINE_PENDING_ACTION' };
		}
	}

	// Default to existing intent detection logic
	// You can expand this to include more sophisticated intent detection
	...
	if (
		...
		return { mode: 'CHAT', intent: 'GET_BUDGET_STATUS' };
	}
	if (
		...
		return { mode: 'CHAT', intent: 'GET_BALANCE' };
	}
	if (lowerText.includes('subscription') || lowerText.includes('recurring')) {
		return { mode: 'CHAT', intent: 'LIST_SUBSCRIPTIONS' };
	}
	if (
		...
		return { mode: 'CHAT', intent: 'FORECAST_SPEND' };
	}
	if (
		...
		return { mode: 'CHAT', intent: 'CATEGORIZE_TX' };
	}
