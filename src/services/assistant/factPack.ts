...
export class FactPackCalculator {
	...
	static validateFactPack(factPack: FactPack): {
		isValid: boolean;
		errors: string[];
	} {
		const errors: string[] = [];
		...
		factPack.budgets?.forEach((budget, index) => {
			const calculatedUtilization = this.calculateUtilization(
				budget.spent,
				budget.limit
			);
			if (Math.abs(budget.utilization - calculatedUtilization) > 1) {
				errors.push(
					`Budget ${index}: utilization mismatch (${budget.utilization} vs ${calculatedUtilization})`
				);
			}
		...
		factPack.goals?.forEach((goal, index) => {
			const calculatedProgress = this.calculateGoalProgress(
				goal.currentAmount,
				goal.targetAmount
			);
			if (Math.abs(goal.progress - calculatedProgress) > 1) {
				errors.push(
					`Goal ${index}: progress mismatch (${goal.progress} vs ${calculatedProgress})`
				);
			}
...
export class FactPackBuilder {
	...
	setBudgets(budgets: FactPack['budgets']): this {
		...
		this.factPack.budgets = budgets.map((budget) => ({
			...
			utilization: FactPackCalculator.calculateUtilization(
				budget.spent,
				budget.limit
			),
			status: FactPackCalculator.determineBudgetStatus(
				budget.spent,
				budget.limit
			),
		}));
	...
	setGoals(goals: FactPack['goals']): this {
		...
		this.factPack.goals = goals.map((goal) => ({
			...goal,
			progress: FactPackCalculator.calculateGoalProgress(
				goal.currentAmount,
				goal.targetAmount
			),
			remaining: goal.targetAmount - goal.currentAmount,
			status: FactPackCalculator.determineGoalStatus(
				goal.currentAmount,
				goal.targetAmount,
				goal.deadline
			),
