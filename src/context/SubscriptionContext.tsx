	...
	useEffect(() => {
		...
		const initRevenueCat = async () => {
			try {
				...
				if (err?.message?.includes('not linked') || err?.message?.includes('doesn\'t seem to be linked')) {
					setError('RevenueCat SDK requires a rebuild. Please rebuild the app after installing native packages.');
				} else {
		...
		initRevenueCat();
	}, [firebaseUser?.uid, refreshSubscription]);
