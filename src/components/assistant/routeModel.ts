...
import { EnhancedCriticService, CriticValidation } from './enhancedCriticService';
	...
	try {
		switch (intent) {
			...
			case 'FORECAST_SPEND':
				...
				break;
			default:
				facts.push('General financial context available');
		}
...
export async function miniModelWrite(
	...
	switch (intent) {
		...
		case 'FORECAST_SPEND':
			...
			break;
		default:
			message = `Here's what I found: ${facts.join('. ')}.`;
	}
...
export async function miniCriticValidate(
	...
	if (factPack && context) {
		try {
			...
			const validation = await enhancedCritic.validateResponse(message, query, context);
			
			// Log which guard failed for analytics clustering
			if (!validation.isValid && validation.ruleValidation.guardFailed) {
				console.log('🔍 [EnhancedCritic] Guard failed:', validation.ruleValidation.guardFailed);
			}
			
			return validation;
		} catch (error) {
			console.warn('🔍 [EnhancedCritic] Enhanced critic failed, falling back to basic:', error);
		}
	}
	...
	return {
		...
		claimTypes: {
			...
			forbiddenClaims,
			riskLevel: forbiddenClaims.length > 2 ? 'high' : forbiddenClaims.length > 0 ? 'medium' : 'low'
		},
		confidence,
		tokenCount,
...
export async function executeHybridCostOptimization(
	...
	if (criticValidation.escalationTriggered) {
		console.log('🔍 [EnhancedCritic] Escalating to Pro model:', criticValidation.escalationReason);
		try {
