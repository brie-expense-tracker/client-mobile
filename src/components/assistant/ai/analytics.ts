...
export class CascadeAnalyticsService {
	 ...
	 */
	private mockAnalyticsCall(eventName: string, data: any) {
	 ...
	 */
	getSessionSummary() {
		const writerEvents = this.events.filter((e) => 'used_fact_ids' in e);
		const guardEvents = this.events.filter((e) => 'failures' in e);
		const criticEvents = this.events.filter((e) => 'risk' in e);
		const decisionEvents = this.events.filter((e) => 'path' in e);
		const improverEvents = this.events.filter((e) => 'escalation_reason' in e);
	 ...
	 */
	getFailureAnalysis() {
		const guardFailures = this.events
			.filter((e) => 'failures' in e && e.failures.length > 0)
		...
		const criticIssues = this.events
			.filter((e) => 'issues' in e && e.issues.length > 0)
			.flatMap((e) => e.issues.map((i) => i.type));
...
export function logCascadeEvent<T extends keyof CascadeAnalytics>(
	...
	switch (eventType) {
		case 'writer_done':
			cascadeAnalytics.logWriterDone(data as any);
			break;
		case 'guard_fail':
			cascadeAnalytics.logGuardFail(data as any);
			break;
		case 'critic_done':
			cascadeAnalytics.logCriticDone(data as any);
			break;
		case 'decision':
			cascadeAnalytics.logDecision(data as any);
			break;
		case 'improver_used':
			cascadeAnalytics.logImproverUsed(data as any);
			break;
		case 'user_outcome':
			cascadeAnalytics.logUserOutcome(data as any);
			break;
	}
}
