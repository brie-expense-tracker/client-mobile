...
describe('EnhancedCriticService', () => {
  ...
  describe('validateResponse', () => {
    it('should pass validation for valid response', async () => {
      ...
      const result = await criticService.validateResponse(message, query, context);
    ...
    it('should fail validation for negative amounts', async () => {
      ...
      expect(result.ruleValidation.guardFailed).toBe('numeric_negative_amounts');
    ...
    it('should fail validation for forbidden claims', async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.ruleValidation.passed).toBe(false);
      expect(result.ruleValidation.guardFailed).toBe('claim_forbidden_phrasing');
      expect(result.escalationTriggered).toBe(true);
    });
    ...
    it('should detect unresolved ambiguity', async () => {
      const message = 'Maybe you should save more, or perhaps spend less. It depends on your situation.';
    ...
    it('should detect potential hallucination', async () => {
      const message = 'According to your data, your average spending is $2,500 per month.';
  ...
  describe('claim type validation', () => {
    it('should detect high-risk forbidden claims', async () => {
      const message = 'Guaranteed returns! Risk-free investment! Surefire profits!';
