...
export function composeStructuredResponse(
	...
	return {
		message: narration,
		summary:
			narration.length > 100 ? narration.substring(0, 100) + '...' : narration,
		details: narration,
		cards: [],
...
function extractInsightsFromNarration(narration: string): string[] {
	...
	if (insights.length === 0) {
		insights.push('General financial context available');
	}
...
function generateDefaultActions(
	...
	if (context?.hasGoals) {
		actions.push({ label: 'Check Goals', action: 'OPEN_BUDGETS' });
	}
